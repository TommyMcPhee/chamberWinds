...
public:
	...
	void setup();
	std::array<std::atomic<float>, 2> compared_amplitude, compared_delta, compared_pitch;
	void draw();
	...
	void analysis(float z1, float z0, float &dc, float &amplitude_root, float &amplitude, float &delta, bool &cross, float &cross_count, float &pitch);
